...
/// @title KERNEL OFT Adapter Interface
/// @notice Interface for the KERNEL OFT (Omnichain Fungible Token) Adapter, which integrates with LayerZero protocol
interface IKERNEL_OFTAdapter {
    /// @notice Error message for when the native fee and/or LZ token fee provided is insufficient
    error LZ_InsufficientFee(
        uint256 requiredNativeFee, uint256 suppliedNativeFee, uint256 requiredLzTokenFee, uint256 suppliedLzTokenFee
    );
